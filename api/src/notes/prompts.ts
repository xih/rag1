...
export type ArxivPaperNote2 = {
  note: string;
  pageNumbers: number[];
};

export const outputParser2 = (
  output: BaseMessageChunk
): Array<ArxivPaperNote2> => {
  const tool_calls = output.additional_kwargs.tool_calls;

  if (!tool_calls || tool_calls.length === 0) {
    throw new Error("No tool calls");
  }
  const notes: Array<ArxivPaperNote2> = tool_calls
    .map((tool_call) => {
      const { notes } = JSON.parse(tool_call.function.arguments);
      return notes;
    })
    .flat();
  return notes;
